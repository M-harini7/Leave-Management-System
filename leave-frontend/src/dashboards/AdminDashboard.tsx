  ...
  return (
    <div className="flex h-screen bg-white">
      ...
      <aside className="w-64 bg-gray-900 text-gray-100 p-6 rounded-r-lg shadow-md flex flex-col">
        <h1 className="text-2xl font-extrabold mb-8 tracking-wide">Admin Dashboard</h1>
        <nav className="flex flex-col gap-4 flex-grow">
          <button
            onClick={() => setActiveTab('users')}
            className={`text-left px-4 py-3 rounded-md font-semibold transition-colors duration-300
              ${activeTab === 'users' ? 'bg-gray-700 shadow-inner' : 'hover:bg-gray-700/80'}`}
          >
            Users
          </button>
          <button
            ...
            className={`text-left px-4 py-3 rounded-md font-semibold transition-colors duration-300
              ${activeTab === 'leaves' ? 'bg-gray-700 shadow-inner' : 'hover:bg-gray-700/80'}`}
          >
            Leave Summary
          </button>
          <button
            ...
            className={`text-left px-4 py-3 rounded-md font-semibold transition-colors duration-300
              ${activeTab === 'settings' ? 'bg-gray-700 shadow-inner' : 'hover:bg-gray-700/80'}`}
          >
            Manage Settings
          </button>
        </nav>
      ...
      <main className="flex-1 p-8 overflow-y-auto bg-white">
        ...
        {activeTab === 'settings' && <ManageSettingsPage />}
      </main>
    </div>
  );
};
